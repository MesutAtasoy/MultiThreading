...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            var regularOne = Perfomance(() => new MatricesMultiplier().Multiply(m1, m2));
            var parallelOne = Perfomance(() => new MatricesMultiplierParallel().Multiply(m1, m2));

            Assert.IsTrue(regularOne > parallelOne);

        }

        double Perfomance(Action func)
        {
            Stopwatch sw = new Stopwatch();
            sw.Start();
            func();
